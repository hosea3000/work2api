...
### Requirement: 凭证列表展示
系统 SHALL 提供按 provider 分离的凭证列表端点：`GET /api/admin/codebuddy/credentials` 与 `GET /api/admin/trae/credentials`，各自返回该 provider 的凭证列表（脱敏：bearer_token 仅展示尾部摘要）与该 provider 当前使用中的凭证信息。列表项含：id、user_id、status、auth_source、created_at、签到最后状态、quota 展示字段（可为空）。codebuddy 端点响应含 `auto_rotation_enabled`；TRAE 端点同。两个列表 MUST NOT 混合对方 provider 的凭证。
...
**Reason**: 两 provider 已分表存储，`provider` 列与"列表展示 provider"不再需要——provider 由端点路径决定。
**Migration**: 无存量迁移（清库重建）。

### Requirement: codebuddy 调度路径排除 TRAE 凭证
**Reason**: 两 provider 的存储、池、服务、API 已物理隔离，不再存在"误选对方凭证"的可能，该防御性要求失去意义。
**Migration**: 无。
