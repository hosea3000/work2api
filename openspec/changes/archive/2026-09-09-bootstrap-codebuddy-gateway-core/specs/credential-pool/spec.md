# Spec: credential-pool

## ADDED Requirements

### Requirement: 凭证 SQLite 存储
凭证 MUST 存储于 SQLite `credential` 表，字段包含：id（UUID）、bearer_token、user_id、account_uid、domain、enterprise_id、department_full_name、auth_source（manual）、status（active/expired/disabled）、created_at、updated_at、expires_at。
